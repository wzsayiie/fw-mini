...
static void MVector_insert   (const MGenericVector::ptr &a, int i, const any &v) { a->_insert     (i, v); }
static void MVector_erase    (const MGenericVector::ptr &a, int i)               { a->_erase      (i)   ; }
static void MVector_push_back(const MGenericVector::ptr &a, const any &v)        { a->_push_back  (v)   ; }
static void MVector_pop_back (const MGenericVector::ptr &a)                      { a->_pop_back   ( )   ; }
static void MVector_clear    (const MGenericVector::ptr &a)                      { a->_clear      ( )   ; }
...
static void MMap_insert(const MGenericMap::ptr &a, const any &k, const any &v) { a->_insert      (k, v); }
static void MMap_erase (const MGenericMap::ptr &a, const any &k)               { a->_erase       (k)   ; }
static void MMap_clear (const MGenericMap::ptr &a)                             { a->_clear       ( )   ; }
static bool MMap_has   (const MGenericMap::ptr &a, const any &k)               { return a->_has  (k)   ; }
static any  MMap_get   (const MGenericMap::ptr &a, const any &k)               { return a->_at   (k)   ; }
static int  MMap_size  (const MGenericMap::ptr &a)                             { return a->_size ( )   ; }
static void MMap_begin (const MGenericMap::ptr &a)                             { a->_begin       ( )   ; }
static bool MMap_on    (const MGenericMap::ptr &a)                             { return a->_on   ( )   ; }
static void MMap_next  (const MGenericMap::ptr &a)                             { a->_next        ( )   ; }
...
static void MSet_insert(const MGenericSet::ptr &a, const any &v) { a->_insert      (v); }
static void MSet_erase (const MGenericSet::ptr &a, const any &v) { a->_erase       (v); }
static void MSet_clear (const MGenericSet::ptr &a)               { a->_clear       ( ); }
...
static void MSet_begin (const MGenericSet::ptr &a)               { a->_begin       ( ); }
static bool MSet_on    (const MGenericSet::ptr &a)               { return a->_on   ( ); }
static void MSet_next  (const MGenericSet::ptr &a)               { a->_next        ( ); }
